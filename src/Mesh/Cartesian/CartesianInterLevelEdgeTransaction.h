...
namespace cmf
{
    ///@brief A class representing a data transaction between two blocks of differing refinement levels, oriented on an edge
    ///@author WVN
    template <typename numType> class CartesianInterLevelEdgeTransaction : public CartesianInterLevelBlockTransaction<numType>
    {
        ...
        public:
            ...
            ///@author WVN
            CartesianInterLevelEdgeTransaction
                (
                ...
                for (int k = kmin; k < kmax; k++)
                {
                    for (int j = jmin; j < jmax; j++)
                    {
                        for (int i = imin; i < imax; i++)
                        {
                            ...
                            {
                                recvInfo.array(v, i+di, j+dj, k+dk) = 50.0;
                            }
                        }
                    }
                }
